program asteroid_belt_simulation
  ...
  real, parameter :: M_SUN = 1.989E30
  real, parameter :: AU = 1.496E11
  real, parameter :: DAY = 86400.0
  integer, parameter :: N_AST = 100
  integer, parameter :: N_STEPS = 720
  real, parameter :: DT = DAY / 24.0

  ! Variables
  real :: x(N_AST), y(N_AST), z(N_AST)
  real :: vx(N_AST), vy(N_AST), vz(N_AST)
  real :: ax(N_AST), ay(N_AST), az(N_AST)
  real :: r, t
  integer :: i, step
...
contains
  ...
  subroutine initialize_asteroids(x, y, z, vx, vy, vz)
    ...
    do i = 1, N_AST
      ...
      theta = 2.0 * 3.14159 * real(i-1)/(N_AST-1)
      x(i) = r * cos(theta)
      y(i) = r * sin(theta)
      z(i) = 0.0
      v_circ = sqrt(G * M_SUN / r)
      vx(i) = -v_circ * sin(theta)
